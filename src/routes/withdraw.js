...
function Withdraw() {
    ...
    function handleSubmit() {
        ...
        /* Display error if the requested amount exceeds the available balance */
        if (currentBalance < Number(withdraw)) {
            setMessage(`The requested amount exceeds your current balance`);
            setWithdraw(0);
            return;
        }
        /* Display error if the requested amount is a negative number */
        if (withdraw <= 0) {
            ...
            body={show ? (
                <>
                    ...
                    <input type="number" className="form-control" id="withdraw" 
                     placeholder="Enter amount to withdraw" value={withdraw} 
                     onChange={e => {makeChange(e, setWithdraw)}}/><br/>
                    <button type="submit" disabled={!enable} className="btn btn-light" 
                     onClick={handleSubmit}>Withdraw</button><br/><br/>
