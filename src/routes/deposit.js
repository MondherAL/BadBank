...
function Deposit() {
    ...
    function handleSubmit() {
        /* Display error if the deposit is negative */ 
        if (deposit <= 0) {
            ...
            body={show ? (
                <>
                    ...
                    <input type="number" className="form-control" id="deposit" 
                     placeholder="Enter amount to deposit" value={deposit} 
                     onChange={e => {makeChange(e, setDeposit)}}/><br/>
                    <button type="submit" disabled={!enable} className="btn btn-light" 
                     onClick={handleSubmit}>Deposit</button><br/><br/>
